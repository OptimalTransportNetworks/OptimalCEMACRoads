...
WC24_CEMAC <- WC24_Africa |> subset(iso3 %in% CEMAC) 
WC24_CEMAC |> count(iso3)

# Weaving in administrative information through power weights
WC24_CEMAC[, pop_cap := 6^as.integer(factor(capital, levels = c("", "admin", "minor", "primary"))) * population]
# First Deduplication: extra weight for administrative function
WC24_CEMAC_red <- largest_within_radius(WC24_CEMAC, c("lon", "lat"), "pop_cap", radius_km = 30) 
WC24_CEMAC_red <- largest_within_radius(WC24_CEMAC_red, c("lon", "lat"), "pop_cap", radius_km = 60)  

# Plotting
# WC24_CEMAC_red |> with(plot(lon, lat, cex = sqrt(population)/1e3, pch = 16, col = rgb(0, 0, 0, alpha=0.5)))
# WC24_CEMAC_red |> st_as_sf(coords = c("lon", "lat"), crs = st_crs(4326)) |> select(population, city) |> mapview::mapview()

# Now Adding populations within 30 km
WC24_CEMAC_red <- join_within_radius(WC24_CEMAC_red, WC24_CEMAC, c("lon", "lat"), radius_km = 30)

# Now a second deduplication using the 40k threshold
WC24_CEMAC_g40k_red <- largest_within_radius(WC24_CEMAC_red[population > 4e4], c("lon", "lat"), "population", radius_km = 70) 
# WC24_CEMAC_g40k_red |> with(plot(lon, lat, cex = sqrt(population)/1e3, pch = 16, col = rgb(0, 0, 0, alpha=0.5)))
# WC24_CEMAC_g40k_red |> st_as_sf(coords = c("lon", "lat"), crs = st_crs(4326)) |> select(city, population) |> mapview::mapview()
...
WBP_CEMAC <- fread("data/other_inputs/continental_african_ports.csv") |>
             subset(iso3c %in% CEMAC)
WBP_CEMAC_g100k <- WBP_CEMAC |> subset(outflows > 1e5)

## Visualization
# WBP_CEMAC_g100k |> with(plot(lon, lat, cex = sqrt(outflows)/1e3, pch = 16, col = rgb(0, 0, 0, alpha=0.5)))
# WBP_CEMAC_g100k |> st_as_sf(coords = c("lon", "lat"), crs = st_crs(4326)) |> select(outflows) |> mapview::mapview()

# Deduplication
WBP_CEMAC_g100k_red <- largest_within_radius(WBP_CEMAC_g100k, size = "outflows", radius_km = 100)
WBP_CEMAC_g100k_red <- join_within_radius(WBP_CEMAC_g100k_red, WBP_CEMAC, size = "outflows", radius_km = 99.9)

## Visualization
# WBP_CEMAC_g100k_red |> with(plot(lon, lat, cex = sqrt(outflows)/1e3, pch = 16, col = rgb(0, 0, 0, alpha=0.5)))
# WBP_CEMAC_g100k_red |> st_as_sf(coords = c("lon", "lat"), crs = st_crs(4326)) |> select(outflows) |> mapview::mapview()

# Now Joining Cities and Ports ---------------------------------

cities_ports_dmat <- st_distance(with(WC24_CEMAC_g40k_red, s2_lnglat(lon, lat)), with(WBP_CEMAC_g100k_red, s2_lnglat(lon, lat)))
...
settransform(WBP_CEMAC_g100k_red, row = m_ind)
settransform(WC24_CEMAC_g40k_red, row = seq_along(city))

WC24_CEMAC_g40k_red <- WC24_CEMAC_g40k_red |> 
  join(subset(WBP_CEMAC_g100k_red, !is.na(row), row, port_locode = locode, port_name = name, port_status = status, outflows), on = "row") |> 
  select(-row)
rm(m_ind)

# Manual Edits
cities_ports <- WC24_CEMAC_g40k_red |>
  subset(city_ascii %!in% c("Malabo", "Faya"))
...
for (i in seq_row(routes_ind)) {
  ...
  if(is.null(route)) {
    cat(sprintf("\nroute %d from %s to %s could not be calculated", i, routes$from_city_country[i], routes$to_city_country[i]))
    next    
  }
...
tm_basemap("Esri.WorldGrayCanvas", zoom = 6) +
  ...
  tm_lines(col = "gravity_rd",
           col.scale = tm_scale_intervals(values = "inferno", breaks = c(0, 0.25, 1, 5, 25, 100)),
...
sum(nodes$population) / sum(WC24_CEMAC$population)
...
save(nodes, edges, edges_ind, nodes_coord, net, add_links, WC24_CEMAC, 
